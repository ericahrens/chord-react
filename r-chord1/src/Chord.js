...
export class NoteSet {
    constructor(name, offsets, baseName, rootNote) {
        this.name = name
...
for(let base=0;base<12;base++) {
    for(const chord of chords) {
        allChords.push(new NoteSet(`${notes[base]} ${chord.getName()}`, chord.getOffsets(base), chord.getName(), base))
    }
}
...
export const getMatchingChords = (scale) => {
...
export const getScale = (name) => {
    return scales.filter((scale) => scale.name.startsWith(name))[0]
}

export const deriveNoteSet = (baseNote, scale) => {
